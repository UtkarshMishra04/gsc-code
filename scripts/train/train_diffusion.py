...
from temporal_policies.mixed_diffusion.datasets import get_primitive_loader
...
def train_model(
    ...
    for epoch in tqdm.tqdm(range(num_epochs)):
        ...
        for batch in dataloader_transition:
            ...
            loss = diffusion_model_transition.get_loss(data)
        ...
        for batch in dataloader_state:
            data = batch
            data = data.to(device).float()
            loss = diffusion_model_state.get_loss(data)
        ...
        if verbose:
            print(f"Epoch {epoch + 1}/{num_epochs} State loss: {loss.item():.4f}")
        ...
        if score_model_classifier is not None:
            for batch in dataloader_classifier:
                data = batch
                data = data[:, -score_model_classifier.state_dim:]
                target = batch[:, :-score_model_classifier.state_dim]
                data = data.to(device).float()
                target = target.to(device).float()
        ...
        if (epoch + 1) % 10 == 0:
            if path is not None:
                ...
                torch.save(diffusion_model_state.state_dict(), path / "diffusion_model_state.pt")
...
def train_diffusion(
    ...
    transition_model = TransitionModel(
        sample_dim=dataset_transition[0].shape[0],
        state_dim=dataset_state[0].shape[0],
        out_channels=512
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-epochs", type=int, default=140, help="Number of epochs")
