...
def get_action_from_diffusion(
    ...
    for t in tqdm.tqdm(range(num_steps, 0, -1)):
        ...
        epsilon, alpha_t, alpha_tm1 = sde.sample_epsilon(t * ones, xt)
        ...
        if use_transition_model:
            pred[:, state_dim+action_dim:] = transition_model(pred[:, :state_dim+action_dim])
    ...
    scores = classifier(torch.Tensor(initial_state).to(device)).detach().cpu().numpy().squeeze()
    ...
    for i in pbar:
        ...
        while not done:
            ...
            for j, action in enumerate(actions):
                ...
                predicted_observation = transition_model(torch.Tensor(np.concatenate([obs0, action])).unsqueeze(0).to(device)).detach()
                predicted_score_0 = score_model_classifier(torch.Tensor(obs0).unsqueeze(0).to(device)).detach()
                predicted_score_1 = score_model_classifier(predicted_observation).detach()
                predicted_observation = policy.encoder.decode(predicted_observation, reset_info["policy_args"]).cpu().numpy()
