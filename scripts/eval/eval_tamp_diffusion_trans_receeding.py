...
def get_action_from_multi_diffusion(
    ...
    state_dim: int = 96,
    action_dim: int = 4,
    gamma: Sequence[float] = [1.0, 0.5, 0.5, 1.0, 1.0, 0.0, 1.0, 1.0],
    ...
    for t in tqdm.tqdm(range(num_steps, 0, -1)):
        ...
        if t > 0.25*num_steps:
            action_place = pred_x0[:, 2*(state_dim+action_dim)+state_dim:3*(state_dim+action_dim)]
            # action_rack = pred_x0[:, -state_dim-action_dim:-state_dim]

            for _ in range(5):

                action_place = action_place.clone()

                action_place.requires_grad = True

                # maximize distance from origin

                distance = torch.norm(action_place[:, :2], dim=1, keepdim=True).mean()

                distance.backward()

                action_grad = action_place.grad.detach()

                action_place = action_place.detach()

                action_place[:, :2] = action_place[:, :2] - action_grad[:, :2]

            pred_x0[:, 2*(state_dim+action_dim)+state_dim:3*(state_dim+action_dim)] = action_place
        ...
        for j in range(actions[0].shape[0]):
            ...
            for i, action in enumerate(actions):
                ...
                try:
                    ...
                    encoded_observation = obs_preprocessor(observation.copy(), env.action_skeleton[i].get_policy_args())
                    success_prob = classifier_models[skills[target_skill_sequence[i]]](torch.from_numpy(np.concatenate([encoded_observation, np.array(observation_indices)])).float().to(device).unsqueeze(0)).detach().cpu().numpy()[0]
                    print("affordability_prob:", env.action_skeleton[i], success_prob)
                    observation, reward, terminated, truncated, step_info = env.step(action[j])
                    encoded_observation = obs_preprocessor(observation.copy(), env.action_skeleton[i].get_policy_args())
                    success_prob = classifier_models[skills[target_skill_sequence[i]]](torch.from_numpy(np.concatenate([encoded_observation, np.array(observation_indices)])).float().to(device).unsqueeze(0)).detach().cpu().numpy()[0]
                    print("success_prob:", env.action_skeleton[i], success_prob)
