...
class MixedDiffusionDataset(Dataset):
    ...
    def __init__(self, dataset_path, obs_processor):
        ...
        with open(self.neg_eval_datapath, 'rb') as f:
            ...
            self.neg_data = np.random.permutation(self.neg_data)[:5000]
    ...
    def get_data_for_mode(self, mode):
        ...
        if mode == "transition":
            data = np.concatenate([self.obs1, self.act, self.obs2], axis=1)
        elif mode == "state":
            data = self.obs1[np.random.permutation(len(self.obs1))][:5000]
        elif mode == "classifier":
            pos_data = np.concatenate([self.obs2, np.ones([len(self.obs2), 1])], axis=1)[np.random.permutation(len(self.obs2))][:5000]
            neg_data = np.concatenate([self.neg_obs, np.zeros([len(self.neg_obs), 1])], axis=1)[np.random.permutation(len(self.neg_obs))][:5000]
            data = np.concatenate([pos_data, neg_data], axis=0)

        return data

def get_primitive_loader(dataset_path, obs_processor, modes, batch_size=512):
